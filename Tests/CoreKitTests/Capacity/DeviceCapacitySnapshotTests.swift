...
private extension DeviceCapacitySnapshot {
    ...
    static let macFullyPopulated = DeviceCapacitySnapshot(
...
struct DeviceCapacitySnapshotTests {
    ...
    private enum RatchetLocateError: Error, CustomStringConvertible {
        ...
        var description: String {
            switch self {
            case let .sourcesNotFound(path):
                "CoreKit sources directory not found at \(path)"
            }
        }
    }
}
